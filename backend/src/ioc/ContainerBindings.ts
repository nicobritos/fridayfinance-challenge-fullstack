...
function registerServices(container: Container): void {
  container.bind<AccountService>(TYPES.Services.AccountService).to(AccountServiceImpl).inSingletonScope();
}

function registerRepositories(container: Container): void {
  container.bind<AccountRepository>(TYPES.Repositories.AccountRepository).to(AccountRepositoryImpl).inSingletonScope();
}

function registerExtra(container: Container): void {
  container.bind<PrismaClient>(TYPES.Extra.PrismaClient).toConstantValue(new PrismaClient());
}
